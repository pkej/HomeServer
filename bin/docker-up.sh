...
function all()
{
  ...
  MANIFESTS="docker-compose ${MANIFESTS} -p ${PROJECTNAME} up -d --remove-orphans"
  echo $MANIFESTS
  $MANIFESTS
...
function one()
{
  ...
  if test -f "$FILE_DISABLED"; then
      ...
      MANIFESTS="docker-compose -f ${FILE_ENABLED} -p ${PROJECTNAME} up -d --remove-orphans"
      echo $MANIFESTS
      $MANIFESTS
      #sudo "$BINDIR/permissions.sh" "-u $USERNAME"
      return
  else
      echo "No service $1 to enable"
      return
  fi
}
